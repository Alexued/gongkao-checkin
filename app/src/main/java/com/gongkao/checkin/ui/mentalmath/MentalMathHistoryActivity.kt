...
/** 速算技巧记录。点一组展开每条技巧的自评。结构照抄 FormulaHistoryActivity。 */
class MentalMathHistoryActivity : ListScreen() {

    private val expanded = mutableSetOf<String>()

    override fun title(): CharSequence = getString(R.string.mental_math_records)

    override fun build() {
        val sessions = Repo.mentalMathSessions()
        if (sessions.isEmpty()) {
            empty()
            return
        }
    ...
    private fun group(s: MentalMathSession) {
    ...
    private fun details(s: MentalMathSession) {
        val box = card(14)
